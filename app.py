...
client = Groq(api_key="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")
...
@app.route('/professor-dashboard/edit/<int:index>', methods=['POST'])
def edit_reminder(index):
    ...
    if index < len(reminders):
        # Update the reminder
        reminders[index]['type'] = request.form.get('reminder_type')
        reminders[index]['description'] = request.form.get('description')
        reminders[index]['due_date'] = request.form.get('due_date')
...
@app.route('/professor-dashboard/delete/<int:index>', methods=['GET'])
...
def professor_reminder_webhook():
    try:
        ...
        # Check for greetings and thanks responses
        if 'hi' in message or 'hello' in message:
            # Assuming you have stored professor names in session or a database
            professor_name = session.get('professor_name', 'Professor')
            return jsonify({'fulfillmentText': f"Hi Mr. {professor_name}, how can I assist you today?"})

        if 'thank you' in message or 'thanks' in message:
            return jsonify({'fulfillmentText': "You're welcome! If you need anything else, feel free to ask."})

        # Existing functionality to process reminders
        reminders = reminders_data['professors'].get(professor_id, {}).get('reminders', [])
    ...
    if best_score > 75:
        ...
        try:
            response = client.chat.completions.create(
                ...
                messages=[{"role": "user", "content": f"Answer in 1-2 sentences: {message}"}]
            )
            return jsonify({'fulfillmentText': response.choices[0].message.content})
        except Exception as e:
            logging.error(f"Groq error: {e}")
            return jsonify({'fulfillmentText': "Error fetching response."})
